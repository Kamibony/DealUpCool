...
async def handle_call_selection(
    ...
    try:
        ...
        if result["status"] == "error" or result["status"] == "info":
            ...
            use_markdown = state_code != -1  # Použít Markdown jen pro potvrzení zájmu

            # !! SPRÁVNÉ ODSAZENÍ ZDE !!
            if state_code == -1:  # Končíme (účast potvrzena bez dat)
                # Pro jistotu načteme detaily znovu pro formátování
                # (I když by měly být už v result['message'] z bot_logic)
                ...
                if call_details:
                    ...
                    try:
                        ...
                        logger.error(f"Chybějící klíč '{e}' při formátování (no data).")
                        # Použijeme zprávu z result, která by měla být bezpečná
                        final_message = result["message"]
                else:
                    final_message = result["message"]  # Základní potvrzení

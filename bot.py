    ...
    try:
        ...
        if result["status"] == "error" or result["status"] == "info":
            ...
            if state_code == ASKING_DATA:  # 0
                ...
                next_state = ConversationHandler.END
                # Vyčistíme user_data
                for key in list(context.user_data.keys()):  # Odsazení: 8 mezer
                    if key.startswith("current_") or key in [
                        "data_needed_list",
                        "data_needed_index",
                        "collected_data_so_far",
                    ]:  # Odsazení: 12 mezer
                        context.user_data.pop(key, None)  # Odsazení: 16 mezer
        else:
            logger.error(
        ...
        try:  # !! SPRÁVNÉ ODSAZENÍ ZDE !! (Odsazení: 9 mezer -> opraveno na 12)
            await context.bot.send_message(

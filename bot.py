    ...
    try:
        ...
        if result["status"] == "error" or result["status"] == "info":
            ...
            if state_code == ASKING_DATA:  # 0
                ...
                next_state = ConversationHandler.END
                # Správné odsazení pro čištění user_data
        ...
        # Odsazený blok pro odeslání chybové zprávy
        try:
            await context.bot.send_message(
        ...
        # !! OPRAVA ODSAZENÍ ZDE !!
        try:  # Tento try blok musí být odsazený pod `except Exception as e:`
            await context.bot.send_message(
                chat_id=query.message.chat_id,
                text="Neočekávaná chyba při zpracování vaší volby.",
            )
        except Exception as send_e:  # Tento except patří k vnitřnímu try
            logger.error(
                f"HANDLER: Nepodařilo se odeslat ani chybovou zprávu uživateli {user_id}: {send_e}"
            )
        return (
            ConversationHandler.END
        )  # Tento return patří k vnějšímu except Exception as e:
...
async def ask_next_data(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if current_index >= len(needed_list):
        ...
        for key in list(user_data.keys()):  # Správné odsazení
            if key.startswith("current_") or key in [
                "data_needed_list",
                "data_needed_index",
                "collected_data_so_far",

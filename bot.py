...
async def handle_call_selection(
    ...
    try:
                ...
                for key in list(context.user_data.keys()):
                    if key.startswith("current_") or key in [
                        ...
                        context.user_data.pop(
                            key, None
                        )  # odsazeno 9 mezerami od začátku řádku
        else:
            logger.error(
        ...
        try:  # odsazeno 8 mezerami
            await context.bot.send_message(
                chat_id=query.message.chat_id, text="Chyba při zpracování volby."
            )
        except Exception as send_e:  # odsazeno 8 mezerami
            logger.error(
                f"HANDLER: Nepodařilo se odeslat ani chybovou zprávu uživateli {user_id}: {send_e}"
            )
        return ConversationHandler.END  # odsazeno 8 mezerami
    except Exception as e:
        logger.error(
            f"HANDLER: Neočekávaná chyba při handle_call_selection {callback_data} pro user {user_id}: {e}"
        )
        try:  # !! SPRÁVNÉ ODSAZENÍ ZDE !! (odsazeno 9 mezerami)
            await context.bot.send_message(
                chat_id=query.message.chat_id,
                text="Neočekávaná chyba při zpracování vaší volby.",
            )
        except Exception as send_e:  # odsazeno 9 mezerami
            logger.error(
                f"HANDLER: Nepodařilo se odeslat ani chybovou zprávu uživateli {user_id}: {send_e}"
            )
        return ConversationHandler.END  # odsazeno 8 mezerami (patří k vnějšímu except)
...
async def ask_next_data(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if current_index >= len(needed_list):
        ...
        # !! SPRÁVNÉ ODSAZENÍ ZDE !!
        for key in list(user_data.keys()):  # odsazeno 8 mezerami
            if key.startswith("current_") or key in [
                "data_needed_list",
                "data_needed_index",
                "collected_data_so_far",
            ]:  # odsazeno 12 mezerami
                user_data.pop(key, None)  # odsazeno 16 mezerami
        return ConversationHandler.END  # odsazeno 8 mezerami
    else:
...
async def cancel_conversation(
    ...
    # !! SPRÁVNÉ ODSAZENÍ ZDE !!
    for key in list(user_data.keys()):  # odsazeno 4 mezerami
        if key.startswith("current_") or key in [
            "data_needed_list",
            "data_needed_index",
            "collected_data_so_far",
        ]:  # odsazeno 8 mezerami
            user_data.pop(key, None)  # odsazeno 12 mezerami
    return ConversationHandler.END  # odsazeno 4 mezerami
...
# --- Handler pro /moje_ucasti --- (Definice zde)
async def my_participations_command(
    ...
    for part in active_participations:
        call_name = part.get(
            "call_name", f"Výzva ID {part.get('call_id', '?')}"
        )  # Bezpečnější přístup
        status_cz = status_translation.get(
            part.get("status", ""), part.get("status", "Neznámý")
        )
...
async def handle_unknown_message(
    ...
    if "current_data_key" in context.user_data:
        logger.info(f"User {user_id} poslal '{text}' během sběru dat.")
        await update.message.reply_text(
            "Prosím, odpověz na otázku nebo zruš pomocí /cancel."
        )
    else:
        logger.warning(
...
def main() -> None:
    ...
    application.add_handler(
        CommandHandler("moje_ucasti", my_participations_command)
    )  # Registrace /moje_ucasti
    application.add_handler(CommandHandler("test", test_command))  # Registrace /test
    application.add_handler(
        MessageHandler(

...
async def my_participations_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # (kód my_participations_command zůstává stejný)
    ...
    for part in active_participations: call_name = part.get('call_name', f"Výzva ID {part.get('call_id', '?')}"); status_cz = status_translation.get(part.get('status', ''), part.get('status', 'Neznámý')); message_parts.append(f"\n- *{call_name}*"); message_parts.append(f"  Stav: {status_cz}")
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("test", test_command)) # <<< Registrace testovacího příkazu
